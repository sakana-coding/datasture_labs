...
MyQueue::MyQueue()
    : data_(new ValueType[kInitialCapacity]), capacity_(kInitialCapacity), front_(0), rear_(0), size_(0) {}
...
MyQueue& MyQueue::operator=(const MyQueue& other) {
    ...
    ValueType* newData = new ValueType[other.capacity_];
...
void MyQueue::append(ValueType value) {
    if (size_ == capacity_) {
        expand();
    }
...
bool MyQueue::retrieve(ValueType& value) const {
    if (empty()) {
        return false;
    }
